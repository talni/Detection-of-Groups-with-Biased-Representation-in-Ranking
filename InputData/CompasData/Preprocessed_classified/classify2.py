...
def classify(data, datafile_prefix):
    ...
    for index, row in X_test_original.iterrows():
        if clf.predict([row.to_list()]) != y_test_original.loc[index]:
            num = num + 1
            # print ('-----------------')
            # print (str(num) + '\n')
            # print (row)
    ...
    print(num)
    print("accuacy", (1 - num / len(X_test_original)) * 100)  # accuracy

    X_test_original_res = X_test_original.copy()
    X_test_original_res['act'] = y_test_original  # actual result
    X_test_original_res['pred'] = X_test_original.apply(lambda x: clf.predict([x.to_list()]), axis=1)
