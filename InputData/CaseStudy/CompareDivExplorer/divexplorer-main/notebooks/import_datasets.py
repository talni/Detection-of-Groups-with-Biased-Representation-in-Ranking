...
def train_predict(dfI, type_cl="RF", labelEncoding=True, validation="train", k_cv=10, args={}, retClf=False, fold="stratified"):
    ...
    if validation!="cv":
        ...
        if fold=="stratified":
            ...
            cv = StratifiedKFold(n_splits=k_cv, random_state=None)        #Aggiunto per fissare il random state
        else:
    ...
    if validation!="cv":
        ...
        y_predict_prob = cross_val_predict(clf, X, y["class"].values.ravel(), cv=cv, method='predict_proba')
